...
| `AGERA5` | Global (1979–present), incl. poles | ~0.1° (~10 km) daily | AgERA5 **v2.0** (ECMWF agrometeorological reanalysis for agriculture; all variables, higher-res than NASA POWER, covers high latitudes). The default `timeseries` backend caches compact all-variable CSVs on globally anchored AgERA5 cells; `gridded` retains the legacy daily-NetCDF ZIP path. **Not keyless**: run `setup_cds_credentials()` or provide `CDSAPI_KEY` / `~/.cdsapirc`, then accept the selected dataset licence once (see below). Requests use a bounded queue (`agera5_max_concurrent_requests`) and validated cache. Requires `cdsapi`+`xarray` (Python) / `ecmwfr`+`terra` (R). |
...
pipelines feed jobs through the bounded queue (`agera5_max_concurrent_requests`,
hard cap of 4 in `dssatutils`), keep time-series chunks within the CDS 5 x 5
degree area limit, and use the shared cache at `agera5_netcdf_cache/` under the engine input
root.
